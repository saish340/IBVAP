    ...
    <div className="min-h-screen bg-[#071014] text-slate-100">
      ...
      <main className="mx-auto grid max-w-[1500px] gap-5 p-5 md:p-8 lg:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.75fr)]">
        <div className="space-y-5">
          <Panel title="Live camera" action={selected && <span className="flex items-center gap-2 text-xs"><span className={`border px-2 py-0.5 font-semibold uppercase tracking-wider ${(degradation?.condition ?? "CLEAR") === "CLEAR" ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300" : "border-amber-400/40 bg-amber-400/10 text-amber-200"}`}>{degradation?.condition ?? "CLEAR"}</span>{adaptive?.mode === "night_enhancement" && <span className="border border-violet-500/40 bg-violet-500/10 px-2 py-0.5 uppercase tracking-wider text-violet-300">CLAHE ON</span>}<span className="text-slate-500">{selected.name} / {selected.running ? "streaming" : "stopped"}</span></span>}>
